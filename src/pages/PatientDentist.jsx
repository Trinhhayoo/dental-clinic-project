...
import { AllPatient, filterEmpGenderRole, deleteEmpID, searchEmp } from "../redux/services/Api";
...
const EmpComponent = ({ emp, handleDeleteClick }) => {
    ...
    return (
        <div>
            ...
            <div className='w-full grid grid-cols-[2fr,2fr,2fr,2fr,2fr,1fr] items-center hover:bg-black-400/50 py-2 p-4 rounded-2xl cursor-pointer mb-2 bg-white'>
                <h3 className='font-bold text-base text-100 '>
                    
                       <Link to ={`/PatientDetail/${emp?.patient_id}`}>
                       {emp?.name}
                       </Link>
                ...
                <p className="text-sm">{emp?.address}</p>
                <p>{emp?.phone}</p>
...
const PatientDentist = () => {
    ...
    useEffect(() => {
        const FetchData = async () => {
            ...
            try {
                const { data: response } = await AllPatient(token);
...
// haha ủa sao m k làm giống t t để tên thường mà, gán cột cho mấy cái attribute đó th
    return (
